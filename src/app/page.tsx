...
import {
  ...
  FiStar,
  FiUser,
} from "react-icons/fi"; // Ícones para contato e botões
...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 md:p-8 bg-game-bg-light pattern-dots pattern-game-bg pattern-bg-fixed pattern-opacity-20">
      <div
        className="pixel-box w-full max-w-3xl text-center shadow-pixel-lg animate-fadeIn bg-game-bg p-6"
      ...
      >
        ...
        <DantonNPC
          npcMessage={t("home.npcMessage")}
          name={portfolioData.name}
        />

        {/* Seções como "Quests" ou "Áreas do Mapa" */}
        <nav className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 mb-8 px-4 md:px-6">
          <QuestButton href="/sobre" icon={<FiUser />}>
            {t("home.questButtons.about")}
          </QuestButton>
          <QuestButton href="/experiencias" icon={<FiBriefcase />}>
            {t("home.questButtons.experiences")}
          </QuestButton>
          <QuestButton href="/projetos" icon={<FiAward />}>
            {t("home.questButtons.projects")}
          </QuestButton>
          <QuestButton href="/habilidades" icon={<FiStar />}>
            {t("home.questButtons.skills")}
          </QuestButton>
        </nav>

        {/* Contato Rápido */}
        <div className="mt-8 pt-6 border-t-2 border-game-border">
          <h3 className="font-pixel text-xl text-game-primary mb-4">
            {t("home.connectTitle")}
          </h3>
          <div className="flex justify-center space-x-5 md:space-x-6">
            <a
              ...
              aria-label="GitHub de Danton Tomacheski"
              className="text-game-text hover:text-game-accent transition-colors transform hover:scale-110"
            >
              <FiGithub size={30} />
            </a>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              aria-label="LinkedIn de Danton Tomacheski"
              className="text-game-text hover:text-game-accent transition-colors transform hover:scale-110"
            >
              <FiLinkedin size={30} />
            </a>
            <a
              ...
              aria-label="Email para Danton Tomacheski"
              className="text-game-text hover:text-game-accent transition-colors transform hover:scale-110"
            >
              <FiMail size={30} />
            </a>
            <a
              href="https://wa.me/5542999689501"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="WhatsApp de Danton Tomacheski"
              className="text-game-text hover:text-game-accent transition-colors transform hover:scale-110"
            >
              <FaWhatsapp size={30} />
            </a>
