...
import { getPortfolioData, Experience } from "../../../data/portfolioData";
import {
  FiBriefcase,
  FiCalendar,
  FiMapPin,
  ...
  return (
    <div className="pixel-box bg-game-bg mb-6 p-6 transform transition-all duration-300 hover:shadow-pixel-lg hover:-translate-y-1">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2">
        ...
        <span className="font-pixel text-sm text-game-text-dark mt-1 sm:mt-0 bg-game-bg-light px-2 py-1 border border-game-border shadow-pixel-sm">
          {exp.period}
        </span>
      </div>
      ...
      <p className="text-game-text-dark leading-relaxed mb-3 whitespace-pre-line">
        {exp.description}
      </p>

      {exp.responsibilities && exp.responsibilities.length > 0 && (
        <div className="mb-3">
          <h4 className="font-pixel text-sm text-game-primary mb-1 flex items-center">
            <FiTool className="mr-2" /> {t("experiences.mainTasks")}
          </h4>
      ...
      <div>
        <h4 className="font-pixel text-sm text-game-primary mb-2 flex items-center">
          <FiTerminal className="mr-2" /> {t("experiences.technologies")}
        </h4>
